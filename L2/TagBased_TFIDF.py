...
def recommend(user, N):
    ...
    # 对Item进行打分，分数为所有的（用户对某标签使用的次数 wut, 乘以 商品被打上相同标签的次数 wti, 除以( 1加上所有用户打某标签的总数 )的自然对数）之和
    ...
    for tag, wut in user_tags[user].items():
        ...
        for item, wti in tag_items[tag].items():
            ...
            if item not in recommend_items:
                recommend_items[item] = wut * wti / log( 1 + sum(tag_users[tag].values()))
            else:
                recommend_items[item] = recommend_items[item] + wut * wti / log( 1 + sum(tag_users[tag].values()))
